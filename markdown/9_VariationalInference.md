...
- a function mapping $$\theta \mapsto q\_{\theta}$$ (denoted above `getq`) together with initial parameters `θ`. This provides more flexibility in the types of variational families that we can use, and can sometimes be slightly more convenient for quick and rough work.
...
For this particular `Model`, we can in fact obtain the posterior of the latent variables in closed form. This allows us to compare both `NUTS` and `ADVI` to the true posterior $$p(s, m \mid \{x\_i\}\_{i = 1}^n )$$.

*The code below is just work to get the marginals $$p(s \mid \{x\_i\}\_{i = 1}^n)$$ and $$p(m \mid \{x\_i\}\_{i = 1}^n$$ from the posterior obtained using ConjugatePriors.jl. Feel free to skip it.*
...
## Copy-paste from [5. Linear regression](5_LinearRegression.ipynb)

This section is basically copy-pasting the code from the [linear regression tutorial](5_LinearRegression.ipynb).
...
```
    ...
    [32m[ADVI] Optimizing...: 100%|█████████████████████████████████████████| Time: 0:00:15[39m
...
*Alternatively, we can instead provide the mapping $$\theta \mapsto q\_{\theta}$$ directly together with initial parameters using the signature `vi(m, advi, getq, θ_init)` as mentioned earlier. We'll see an explicit example of this later on!*
...
```
    ...
    Training set:
        VI loss: 3.0751192965937304
        Bayes loss: 3.071197318378367
        OLS loss: 3.0709261248930275
    Test set: 
        VI loss: 27.18090172028323
        Bayes loss: 27.29732792900136
        OLS loss: 27.094813070760388
...
```
    ...
    MethodError: no method matching prediction(::Array{Float64,1}, ::NamedTuple{(:intercept, :σ₂, :coefficients),Tuple{Array{UnitRange{Int64},1},Array{UnitRange{Int64},1},Array{UnitRange{Int64},1}}}, ::Array{Float64,2})
    Closest candidates are:
      prediction(!Matched::AbstractArray{T,2} where T, ::Any, ::Any) at In[45]:3

    

    Stacktrace:

     [1] (::var"#15#16")(::Int64) at ./none:0

     [2] iterate at ./generator.jl:47 [inlined]

     [3] collect(::Base.Generator{UnitRange{Int64},var"#15#16"}) at ./array.jl:622
...
## Alternative: provide $$\theta \mapsto q\_{\theta}$$ instead of `q` with`update` implemented

As mentioned earlier, it's also possible to just provide the mapping $$\theta \mapsto q\_{\theta}$$ rather than the variational family / initial variational posterior `q`, i.e. use the interface `vi(m, advi, getq, θ_init)` where `getq` is the mapping $$\theta \mapsto q\_{\theta}$$
...
```
    ...
    ┌ Info: [ADVI] Should only be seen once: optimizer created for θ
    │   objectid(θ) = 11287272948633138472
    └ @ Turing.Variational /var/home/tef30/.julia/dev/Turing/src/variational/VariationalInference.jl:161
    [32m[ADVI] Optimizing...: 100%|█████████████████████████████████████████| Time: 0:00:15[39m
...
```
    ...
    [32m[ADVI] Optimizing...: 100%|█████████████████████████████████████████| Time: 0:03:02[39m
...
```
    ...
    12×12 LowerTriangular{Float64,Array{Float64,2}}:
      0.158375       ⋅           ⋅          …    ⋅         ⋅          ⋅      
     -0.00353493    0.176515     ⋅               ⋅         ⋅          ⋅      
     -0.00710011   -0.0513321   0.43868          ⋅         ⋅          ⋅      
     -0.0171229     0.0361298  -0.0402263        ⋅         ⋅          ⋅      
      0.00145917    0.217749   -0.00133804       ⋅         ⋅          ⋅      
      0.0015154    -0.0497224   0.0832017   …    ⋅         ⋅          ⋅      
      0.0196729    -0.0535735   0.00393858       ⋅         ⋅          ⋅      
     -0.0082357    -0.069629    0.104902         ⋅         ⋅          ⋅      
     -0.00276471    0.0160591   0.0947083        ⋅         ⋅          ⋅      
      0.000464556  -0.0528973   0.0709845       0.147862   ⋅          ⋅      
     -0.0061054     0.306239    0.111531    …  -0.124396  0.128692    ⋅      
      0.000928068  -0.142802   -0.106874        0.021658  0.0132652  0.124446
...
```
    ...
    Training set:
        VI loss:        3.0751192965937304
        VI (full) loss: 3.0921350542546553
        Bayes loss:     3.071197318378367
        OLS loss:       3.0709261248930275
    Test set: 
        VI loss:        27.18090172028323
        VI (full) loss: 28.167083468117397
        Bayes loss:     27.29732792900136
        OLS loss:       27.094813070760388
